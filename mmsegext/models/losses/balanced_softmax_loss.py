...
def cross_entropy(pred,
    ...
    if (avg_factor is None) and reduction == 'mean':
        if class_weight is None:
            ...
            label_weights = torch.zeros_like(label, dtype=class_weight.dtype).to(device=class_weight.device)
...
#     return loss
@MODELS.register_module(name='ext-BalancedSoftmaxLoss')
class BalancedSoftmaxLoss(nn.Module):
    """CrossEntropyLoss.

    Args:
        sample_per_class: A int tensor of size [num of classes].
    ...
    def forward(self,
        ...
        if self.class_weight is not None:
            ...
            class_weight = None
        # BalancedSoftmaxLoss
        shape_cls_score = cls_score.shape
        shape_length = len(shape_cls_score)
        shape = [1] * shape_length
        shape[1] = shape_cls_score[1]
        sample_per_class = cls_score.new_tensor(self.sample_per_class).reshape(shape)
        cls_score = cls_score + sample_per_class.log()
        # BalancedSoftmaxLoss end

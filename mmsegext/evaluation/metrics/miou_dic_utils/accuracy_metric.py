...
class AccuracyMetric(Metric):
    ...
    def reduceI(self, value_matrix, q=None):
        active_sum = torch.sum(self.active_classes, dim=1).to(value_matrix)
        if self.binary:
...
class AccuracyMetricList(AccuracyMetric):
    def __init__(self, accuracyD=True,
                 accuracyI=True,
                 accuracyC=True,
                 q=1,
                 binary=False,
                 num_classes=19,
                 ignore_index=255):
        super().__init__(accuracyD=accuracyD, accuracyI=accuracyI, accuracyC=accuracyC, q=q, binary=binary,
                         num_classes=num_classes, ignore_index=ignore_index)
        self.tp_list = []
        self.tn_list = []
        self.fp_list = []
        self.fn_list = []
        self.active_classes_list = []
    ...
    def add(self, pred, label, image_file=None):
        ...
        for i in range(batch_size):
            ...
            self.active_classes_list.append(active_classes)
            self.image_file.append(image_file_i)
        return self.tp_list, self.tn_list, self.fp_list, self.fn_list, self.active_classes_list

    def value(self):
        self.tp = torch.concat(self.tp_list, dim=0)
        self.tn = torch.concat(self.tn_list, dim=0)
        self.fp = torch.concat(self.fp_list, dim=0)
        self.fn = torch.concat(self.fn_list, dim=0)
        self.active_classes = torch.concat(self.active_classes_list, dim=0)
    ...
    def reduceC(self, value_matrix, q=None):
        num_images, num_classes = value_matrix.shape
        active_sum = torch.sum(self.active_classes, dim=0).to(value_matrix)
        if q != None:
            active_sum = torch.max(torch.ones(num_classes).to(value_matrix), (q / 100 * active_sum).to(torch.long))

        indices = torch.arange(num_images).view(-1, 1).expand_as(value_matrix).to(value_matrix)
        mask = indices < active_sum
